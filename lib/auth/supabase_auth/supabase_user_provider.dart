...
class HadiSupabaseUser extends BaseAuthUser {
  HadiSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> hadiSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = HadiSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
